...
def cmodel(cid,nam,_f,_fOut, out_ws, diag):
  ...
  targetmc     = _fin.Get("signal0tag_ttbar")      # define monimal (MC) of which process this config will model
  controlmc_wm = _fin.Get("singlemuon0tag_ttbar")
  controlmc_we = _fin.Get("singleelectron0tag_ttbar")
  ...
  TopScales_wm = targetmc.Clone(); TopScales_wm.SetName("topwm0tag_weights_%s"%cid)
  TopScales_wm.Divide(controlmc_wm); _fOut.WriteTObject(TopScales_wm);

  TopScales_we = targetmc.Clone(); TopScales_we.SetName("topwe0tag_weights_%s"%cid)
  ...
  CRs = [
   Channel("singlemuonw0tagModel",  _wspace,out_ws,cid+'_'+model,TopScales_wm),
   Channel("singleelectronw0tagModel",  _wspace,out_ws,cid+'_'+model,TopScales_we),
  ]
  ...
  addStatErrs(TopScales_wm,CRs[0],'topwm0tag','singlemuonw0tagModel')
  addStatErrs(TopScales_we,CRs[1],'topwe0tag','singleelectronw0tagModel')
