...
def cmodel(cid,nam,_f,_fOut, out_ws, diag):
  ...
  targetmc     = _fin.Get("signal2tag_wjets")      # define monimal (MC) of which process this config will model
  controlmc    = _fin.Get("singlemuon2tag_wjets")  # defines in / out acceptance
  controlmc_e  = _fin.Get("singleelectron2tag_wjets")  # defines in / out acceptance

  # Create the transfer factors and save them (not here you can also create systematic variations of these 
  # transfer factors (named with extention _sysname_Up/Down
  WScales = targetmc.Clone(); WScales.SetName("wmn2tag_weights_%s"%cid)
  WScales.Divide(controlmc);  _fOut.WriteTObject(WScales)  

  WScales_e = targetmc.Clone(); WScales_e.SetName("wen2tag_weights_%s"%cid)
  ...
  CRs = [
   Channel("singlemuon2tagModel",_wspace,out_ws,cid+'_'+model,WScales),
   Channel("singleelectron2tagModel",_wspace,out_ws,cid+'_'+model,WScales_e),
  ]
  ...
  addStatErrs(WScales,CRs[0],'wmn2tag','singlemuon2tagModel')
  addStatErrs(WScales_e,CRs[1],'wen2tag','singleelectron2tagModel')
